function optionChoiceTF(e, parametersArray){
  ...
  var colE = parametersArray[6]
  var row = e.range.getRow();
  var array = ['TRUE', 'FALSE']
  var colArray = [colA, colB, colC, colD, colE]
  var alertText = ''
  var background,
      range,
      values;
  ...
  switch (sheet.getRange(row, questionType).getDisplayValue()){
    case 'True/False':
      ...
      for (var i = 2; i < colArray.length; i++){
        if (values[0][i].length > 0) {
          //call an alert if choiceC OR choiceD OR choiceE text.length > 0 for true/false question
          //setbacground color to red
          sheet.getRange(row, colArray[i]).setBackground('#b30000')
